...
use crate::{
    ...
    lexer::{Kind, LexerCheckpoint, LexerContext, ParserConfig, Token},
};
...
impl<'a, Config: ParserConfig> ParserImpl<'a, Config> {
    ...
    pub(crate) fn checkpoint(&mut self) -> ParserCheckpoint<'a, Config> {
        ParserCheckpoint {
    ...
    pub(crate) fn checkpoint_with_error_recovery(&mut self) -> ParserCheckpoint<'a, Config> {
        ParserCheckpoint {
    ...
    pub(crate) fn rewind(&mut self, checkpoint: ParserCheckpoint<'a, Config>) {
    ...
    pub(crate) fn try_parse<T>(
        &mut self,
        func: impl FnOnce(&mut ParserImpl<'a, Config>) -> T,
    ) -> Option<T> {
    ...
    pub(crate) fn lookahead<U>(
        &mut self,
        predicate: impl Fn(&mut ParserImpl<'a, Config>) -> U,
    ) -> U {
